...
function _smcSweeps(data){ var sw=_smcSwings(data,2),n=data.length,out=[],j; // 전고/전저 꼬리로 찍고 종가는 안으로 복귀 = 유동성 스윕(스탑헌트)
...
function drawStockChart(cv,r){
  ...
  var LON=(r.mk==='COIN'&&window._coinLineOn)?window._coinLineOn:{sr:true,ch:true,tr:true,fib:true,poc:true,ma:true,ob:true,fvg:false,bos:false,liq:false,kz:false,pd:false,ote:false,sweep:false};
  ...
  var CTYPE=(r.mk==='COIN'&&window._chartType)?window._chartType:'candle';
  var LOG=(r.mk==='COIN'&&window._chartLog)?true:false;
  var HAD=data; if(CTYPE==='heikin'){ HAD=[]; var _pho,_phc; for(var _hi=0;_hi<data.length;_hi++){ var _dd=data[_hi], _hc=(_dd[0]+_dd[1]+_dd[2]+_dd[3])/4, _ho=(_hi===0)?((_dd[0]+_dd[3])/2):((_pho+_phc)/2), _hh=Math.max(_dd[1],_ho,_hc), _hl=Math.min(_dd[2],_ho,_hc); HAD.push([_ho,_hh,_hl,_hc,_dd[4],_dd[5]]); _pho=_ho; _phc=_hc; } }
  var _RD=(CTYPE==='heikin')?HAD:data;
  var lo=Math.min.apply(null,_RD.map(d=>d[2])), hi=Math.max.apply(null,_RD.map(d=>d[1]));
  ...
  var gh=priceB-padT;
  var _lhi=Math.log(Math.max(1e-9,hi)), _llo=Math.log(Math.max(1e-9,lo));
  function y(v){ if(LOG){ return padT+(_lhi-Math.log(Math.max(1e-9,v)))/((_lhi-_llo)||1)*gh; } return padT+(hi-v)/((hi-lo)||1)*gh; }
  function xAt(i){ return padL+(i+0.5)*((plotR-padL)/n); }
  // 가격 그리드 + 우측 가격 라벨
  ctx.font='500 17px system-ui,sans-serif';ctx.textBaseline='middle';
  for(var g=0;g<=4;g++){ var yy=padT+gh*g/4, pv=LOG?Math.exp(_lhi-(_lhi-_llo)*g/4):(hi-(hi-lo)*g/4);
  ...
  // 프리미엄/디스카운트(균형선) · OTE 존
  if((LON.pd||LON.ote)&&n>3){ var _rh=-Infinity,_rl=Infinity,_rhi=0,_rli=0,_q; for(_q=0;_q<n;_q++){ if(data[_q][1]>_rh){_rh=data[_q][1];_rhi=_q;} if(data[_q][2]<_rl){_rl=data[_q][2];_rli=_q;} } var _rng=_rh-_rl;
    if(_rng>0&&LON.pd){ var _eq=(_rh+_rl)/2, yeq=y(_eq), yrh=y(_rh), yrl=y(_rl); ctx.fillStyle='rgba(246,70,93,0.06)'; ctx.fillRect(padL,yrh,plotR-padL,yeq-yrh); ctx.fillStyle='rgba(46,189,133,0.06)'; ctx.fillRect(padL,yeq,plotR-padL,yrl-yeq); ctx.strokeStyle='rgba(160,170,190,0.7)'; ctx.setLineDash([6,4]); ctx.lineWidth=1.2; ctx.beginPath();ctx.moveTo(padL,yeq);ctx.lineTo(plotR,yeq);ctx.stroke(); ctx.setLineDash([]); ctx.font='700 12px system-ui,sans-serif'; ctx.textBaseline='middle'; ctx.textAlign='left'; ctx.fillStyle='rgba(246,70,93,0.9)'; ctx.fillText('프리미엄(비싸게)',padL+3,yrh+10); ctx.fillStyle='rgba(160,170,190,0.95)'; ctx.fillText('균형 50%',padL+3,yeq-8); ctx.fillStyle='rgba(46,189,133,0.9)'; ctx.fillText('디스카운트(싸게)',padL+3,yrl-8); }
    if(_rng>0&&LON.ote){ var _up=(_rhi>_rli), a,b; if(_up){ a=_rh-0.79*_rng; b=_rh-0.62*_rng; } else { a=_rl+0.62*_rng; b=_rl+0.79*_rng; } var oy1=y(Math.max(a,b)), oy2=y(Math.min(a,b)); ctx.fillStyle='rgba(224,181,82,0.16)'; ctx.fillRect(padL,oy1,plotR-padL,oy2-oy1); ctx.strokeStyle='rgba(224,181,82,0.7)'; ctx.setLineDash([4,3]); ctx.lineWidth=1.2; ctx.strokeRect(padL,oy1,plotR-padL,oy2-oy1); ctx.setLineDash([]); ctx.font='700 12px system-ui,sans-serif'; ctx.fillStyle='rgba(224,181,82,0.95)'; ctx.textBaseline='bottom'; ctx.textAlign='left'; ctx.fillText('OTE '+(_up?'롱':'숏')+' 0.62–0.79',padL+3,oy1-2); ctx.textBaseline='middle'; } }
  ...
  // 캔들
  if(CTYPE==='line'){ ctx.strokeStyle=r.mk==='COIN'?'#2ebd85':up; ctx.lineWidth=2.4; ctx.beginPath(); for(var j=0;j<n;j++){ var xx=xAt(j), yy=y(data[j][3]); if(j===0)ctx.moveTo(xx,yy);else ctx.lineTo(xx,yy); } ctx.stroke(); }
  else { var _CD=(CTYPE==='heikin')?HAD:data; for(var j=0;j<n;j++){var d=_CD[j],x=xAt(j),rise=d[3]>=d[0],col=rise?up:dn;ctx.strokeStyle=col;ctx.fillStyle=col;ctx.lineWidth=1;
  ...
  // 유동성 스윕(스탑헌트)
  if(LON.sweep){ _smcSweeps(data).slice(-10).forEach(function(sp){ if(sp.i>=n)return; var xx=xAt(sp.i), yy=y(sp.p), col=sp.type==='bear'?'#f6465d':'#2ebd85'; ctx.font='800 12px system-ui,sans-serif'; ctx.fillStyle=col; ctx.textAlign='center'; if(sp.type==='bear'){ ctx.textBaseline='bottom'; ctx.fillText('✕스윕',xx,yy-4); } else { ctx.textBaseline='top'; ctx.fillText('✕스윕',xx,yy+4); } ctx.textBaseline='middle'; ctx.textAlign='start'; }); }
  // BOS / CHoCH (구조 전환)
  ...
  // 크로스헤어가 참조할 기하 상태 저장
  CHART={cv:cv,r:r,data:data,n:n,padL:padL,plotR:plotR,padT:padT,priceB:priceB,volT:volT,volB:volB,RM:RM,hi:hi,lo:lo,emas:emas,W:W,H:H,MA:MA,cw:cw,up:up,dn:dn,sub:sub,line:line,tf:_chartTF,log:LOG};
}
/* ── 차트 크로스헤어 + OHLC 툴팁 ── */
var CHART=null;
...
function drawCrosshair(idx){
  ...
  var d=C.data[idx], yc=C.log?(C.padT+(Math.log(C.hi)-Math.log(Math.max(1e-9,d[3])))/((Math.log(C.hi)-Math.log(Math.max(1e-9,C.lo)))||1)*(C.priceB-C.padT)):(C.padT+(C.hi-d[3])/((C.hi-C.lo)||1)*(C.priceB-C.padT));
...
function homeResolve(scope,s,tf,lab){ if(scope==='coin'){ var sym=(s||'').toUpperCase().replace(/[^A-Z0-9]/g,'').replace(/USDT$/,''); if(!sym)return Promise.resolve(null); var F='https://fapi.binance.com/fapi/v1/'; var itv=tf||'1h';
  ...
  var stf0=tf||'D';
  function _finishStock(it){ if(!it)return null; if(typeof proxyJson!=='function')return null; var isUS=((it.ccy||'').toUpperCase()==='USD'); var stf=stf0; var base='mkt='+(isUS?'US':'KR')+'&code='+encodeURIComponent(it.c)+(isUS?('&exch='+(typeof usExch==='function'?usExch(it.mk):'NAS')):'');
  ...
  var it=_homeStockItem(s); if(it)return Promise.resolve(_finishStock(it));
  // 로컬 풀에 없는 국내 소형주 — 이름→코드 검색(Naver 프록시)
  if(/[가-힣]/.test(s||'')&&typeof proxyJson==='function'){ return proxyJson('/ksearch?q='+encodeURIComponent((s||'').trim())).then(function(j){ var m=(j&&j.items&&j.items[0])||null; if(!m)return null; return _finishStock({c:m.code,n:m.name,mk:m.market||'KR',ccy:'KRW'}); }).catch(function(){return null;}); }
...
function renderCoinLearn(el){
  ...
  /* 🚀 실전 매매법 — 추세 이탈 매매법 (사용자 강의 기반, 교육용 TA · 신호 아님) */
  var svgBreak='<svg viewBox="0 0 220 100" style="width:100%;max-width:320px;height:auto"><rect width="220" height="100" fill="transparent"/><line x1="12" y1="90" x2="150" y2="30" stroke="#e0a83e" stroke-width="2"/><polyline points="16,86 40,70 58,76 82,54 100,60 122,40 140,46 150,38" fill="none" stroke="#2ebd85" stroke-width="2"/><polyline points="150,38 164,52 176,48 190,64 204,60" fill="none" stroke="#f6465d" stroke-width="2"/><circle cx="150" cy="38" r="4" fill="none" stroke="#f6465d" stroke-width="2"/><text x="120" y="96" fill="#e0a83e" font-size="9" font-weight="700">저점 잇기 = 추세선</text><text x="150" y="24" fill="#f6465d" font-size="9" font-weight="700">이탈 첫 캔들</text></svg>';
  html+=cat('🚀 실전 매매법 — 추세 이탈');
  html+=card('① 핵심 아이디어 & 정본 5단계',
    '<div style="display:flex;gap:16px;flex-wrap:wrap;align-items:center"><div style="flex:1;min-width:180px">'
    +'<b>고점·저점은 아무도 모른다</b> → 예측하지 말고 <b>추세(흐름)를 따라간다</b>. 아는 건 하나 — "이 코인은 조정 없이 한 방향으로 가는 중".<br><br>'
    +'<b>정본 5단계</b><br>'
    +'1. <b>힘 좋은 코인</b> 선별(시원하게 오르거나 떨어지는 것).<br>'
    +'2. <b>추세선</b> 긋기 — 상승은 <b>저점끼리</b>, 하락은 <b>고점끼리</b>.<br>'
    +'3. 진행 방향과 <b>같은 방향</b>으로 진입(상승=롱, 하락=숏).<br>'
    +'4. <b>짧게 먹고 빠진다</b>(이익 1 : 손실 2~3).<br>'
    +'5. <b>추세선을 벗어나면 더 이상 하지 않는다</b>(관망).</div>'+svgBreak+'</div>'
    +'<div style="color:var(--faint);font-size:11px;margin-top:8px">봉은 <b>5·15·30분·1시간</b>에서 편한 것. <b>1분봉은 속임수가 많아 비권장.</b> ⚠ 교육용 · 매매 지시 아님.</div>');
  html+=card('② 진입 · 익절 · 손절 (은행처럼 짧게)',
    '추세선 <b>위(롱)/아래(숏)</b>에 있는 동안, 진행 방향으로 계속 얇게 먹습니다.<br>'
    +'· <b>익절(TP)</b> — 진입 기준 <b>+1%</b>면 바로 익절. 레버리지 반영: 3배=3%, 10배=10%.<br>'
    +'· <b>손절(SL)</b> — 진입 기준 <b>−2~3%</b>. 오르는 중에도 약간의 눌림은 있으니 그 정도는 견디고, 그 이상 빠지면 조정이 아닌 <b>하락</b>으로 보고 컷.<br>'
    +'· 진입 → +1% 익절 → 재진입 → +1% … <b>반복</b>. 어차피 짧게 나올 거라 <b>진입 타점은 덜 중요</b>(단, 추세가 살아있어야 함).<br>'
    +'· <b>손익비 1:2~3</b>이라도 <b>거의 다 익절</b>이 전제 — 추세 좋은 코인 올라타면 승률이 높아 <b>기댓값은 +</b>. <b>승률 유지가 생명</b> → 추세 죽으면 즉시 관망.');
  html+=card('③ 이탈 후 판별 — 다시 상승? 손절? (심화)',
    '<div style="color:var(--sub);font-size:12px;margin-bottom:8px">추세선을 이탈해도 <b>다시 오르는 경우가 아주 많습니다.</b> 이탈 후 3패턴을 구분하는 <b>딱 하나의 기준 = 저점을 "어떻게" 이탈하나</b>.</div>'
    +'<b>① 기간조정형(재상승) — 가장 쉬움</b><br>'
    +'저점 안 깨고 <b>평화롭게 옆으로</b>(숨고르기). 조정 시간이 올라온 시간보다 짧고, 추세선만 이탈·저점은 유지 → <b>지지선에서 롱</b>. (조정이 너무 길거나 저점 깨지면 주의)<br><br>'
    +'<b>② 페이크형(저점 털고 재상승) — 가장 어려움</b><br>'
    +'이탈 후 <b>저점을 2~3번 갱신</b>하며 공포를 준 뒤 다시 상승(세력형). 보통 <b>3번째 저점 터는 순간이 진짜 바닥</b> → <b>깨진 저점 자리에서 롱, 스탑은 짧게</b>(하방 열림).<br><br>'
    +'<b>③ 급락형(추세전환) — 빠져야 함</b><br>'
    +'기간조정 없이 <b>장대음봉으로 저점을 짧고 강하게 이탈</b> → 조정이 아니라 하락. <b>롱 버리고 숏 전환</b>(또는 손절·종목 교체).<br><br>'
    +'<div style="background:var(--panel2,#0f151f);border:1px solid var(--line2);border-radius:8px;padding:10px 12px"><b>📌 결정적 신호</b><br>'
    +'· <b>저점을 지지하며 평화롭게</b> 흐름 = 조정 → <b class="up">재상승</b>.<br>'
    +'· <b>장대음봉으로 짧고 강하게</b> 저점 이탈 = <b class="down">하락(전환)</b>.<br>'
    +'※ 변형(리테스트 후 급락)도 같은 기준 — 저점을 강한 음봉으로 깨면 급락.</div>');
  html+=card('④ 리스크 · 심리 (제일 중요)',
    '· <b>익절은 짧게, 손절은 더 짧게.</b> 큰 걸 바라는 순간 진다 — <b>야금야금</b>(홈런 X).<br>'
    +'· <b>한 번에 다 걸지 말 것</b> → 분할 진입·분할 익절.<br>'
    +'· <b>절대 잃지 않기 = 계좌 생존 최우선.</b> 계좌가 살아있어야 다음 기회가 온다.<br>'
    +'· 승률 100%는 없다 — 짧은 손절·익절로 큰 손실을 막고 <b>반복</b>하면 계좌가 불어난다.<br>'
    +'· 힘 좋은 코인 발견하면 멀거니 보다 놓치지 말고 <b>따라 타서</b> 얇게 수익.');
  html+=cat('🧩 패턴 & 전략');
...
async function openCoin(sym){
  ...
  if(!window._coinLineOn){ try{window._coinLineOn=JSON.parse(localStorage.getItem('coinLines'))||null;}catch(e){} if(!window._coinLineOn)window._coinLineOn={sr:true,ch:true,tr:true,fib:true,poc:true,ma:true,ob:true,fvg:false,bos:false,liq:false,kz:false,pd:false,ote:false,sweep:false}; }
  if(window._chartType===undefined){ try{window._chartType=localStorage.getItem('coinChartType')||'candle';}catch(e){window._chartType='candle';} }
  if(window._chartLog===undefined){ try{window._chartLog=(localStorage.getItem('coinChartLog')==='1');}catch(e){window._chartLog=false;} }
  try{
    ...
    var LK=[['sr','지지/저항','#2ebd85'],['ch','채널','#4a9eff'],['tr','추세선','#e0a83e'],['fib','피보','#a06bff'],['poc','매물대','#ff9800'],['ma','이평','#f5a623'],['ob','오더블럭','#22a374'],['fvg','FVG','#26c6da'],['bos','BOS/CHoCH','#ec40a6'],['liq','유동성','#ffa726'],['kz','킬존','#7e57c2'],['pd','프리미엄/디스카운트','#a0aabe'],['ote','OTE','#e0b552'],['sweep','유동성 스윕','#ff5db1']];
...
/* 선 표시 토글(차트만 다시 그림) */
window.toggleCoinLine=function(k){ if(!window._coinLineOn)window._coinLineOn={sr:true,ch:true,tr:true,fib:true,poc:true,ma:true,ob:true,fvg:false,bos:false,liq:false,kz:false,pd:false,ote:false,sweep:false}; window._coinLineOn[k]=(window._coinLineOn[k]===false); try{localStorage.setItem('coinLines',JSON.stringify(window._coinLineOn));}catch(e){}
  var host=$('#coinHost'), LK=['sr','ch','tr','fib','poc','ma','ob','fvg','bos','liq','kz','pd','ote','sweep'];
