...
function _makeLiqEngine(initSym){
  ...
  function draw(){ if(stopped)return; if(!dirty){ requestAnimationFrame(draw); return; } dirty=false; ctx.clearRect(0,0,W,H); var cs=st.candles; if(!cs.length){ requestAnimationFrame(draw); return; }
    ...
    var vp={}, vpMax=0, pocBin=null; for(var i=0;i<n;i++){ var c3=cs[i], q3=c3.v*c3.c; if(q3<=0)continue; var b0=binOf(c3.l), b1=binOf(c3.h), span=Math.max(1,b1-b0+1), per=q3/span; for(var pb=b0;pb<=b1;pb++){ vp[pb]=(vp[pb]||0)+per; if(vp[pb]>vpMax){vpMax=vp[pb];pocBin=pb;} } }
    ...
    for(var i2=0;i2<n;i2++){ var c=cs[i2], x=cl+i2*bw+bw/2, up=c.c>=c.o, col2=up?'#2ebd85':'#f6465d'; ctx.strokeStyle=col2; ctx.fillStyle=col2; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(x,y(c.h)); ctx.lineTo(x,y(c.l)); ctx.stroke(); var bodyW=Math.max(1,bw*0.62), yo=y(c.o), yc=y(c.c); ctx.fillRect(x-bodyW/2,Math.min(yo,yc),bodyW,Math.max(1,Math.abs(yc-yo))); }
  ...
  function start(){ cv=$('#lqChart'); if(!cv)return; ctx=cv.getContext('2d'); var lg=$('#lqLgSym'); if(lg)lg.textContent=st.sym;
